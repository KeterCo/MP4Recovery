    ...
    def process_video(self, input_file: Path, tmp_dir: Path = None):
        ...
        try:
            ...
            if not tmp_output.exists():
                logger.info(f"输出文件未生成: {input_file}")
            ...
            if suffix == '':
                ...
                logger.info(f"成功处理，处理后视频路径: {input_file}")
            else:
                shutil.move(str(tmp_output), str(final_output))
                logger.info(f"成功处理，处理后视频路径: {final_output}")
                # 同步处理关联文件和删除原视频
            ...
            logger.error(f"处理失败（原视频保留）: {input_file}，错误: {str(e)}")
